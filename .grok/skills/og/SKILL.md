...
every app (preview included) shows a favicon in the browser tab. **Share-card
`<meta>` tags are not authored in `__root.tsx`.** The platform injector
(`scripts/grok-pwa-shared.mjs`) overwrites `og:*` and `twitter:card` on every
HTML response. You write identity data only:
...
**Extend `__root.tsx`; never replace it wholesale** (auth SSR, redesign,
another skill's excerpt). Do not add `og:*` or `twitter:card` there — the
injector will replace them. Dropping the favicon link still ships a blank tab
icon, and `npm run dev` / `build` will not catch it.

## `og:type` for games (X game-card presentation)

**When the app is a game of any kind** (Canvas/WebGL *and* DOM board/word/
puzzle/quiz games), set this in `src/lib/og/site.json` — it does not need a
hostname and must not be gated on a custom card:

```json
{ "title": "Sky Strike", "type": "x:game", "card": "custom" }
```
...
| **`twitter:card`** | **Wrong layer for the game signal.** Layout only (`summary_large_image`). The PWA injector always emits it — do not treat this skill as the place that adds it. Never use it as the game type. |
...
canvas app is missing `"type": "x:game"` in `site.json` or (once a custom
link card exists) missing `public/x-banner.jpg` — treat that as not
...
`public/favicon.svg`, `site.json`, PWA icons when installable — needs only
the app's name, theme, and palette.
**Start it as a background task with the `task` tool early in the build**
(AGENTS.md § "Parallel work"), right after the name and palette are settled:
give the task this skill and sole ownership of `public/` brand assets plus
`src/lib/og/site.json`, keep building the app, and merge `site.json` at the
end (`wait_tasks` before the final verify). Image generation is pure waiting
— a build that runs the card pass inline at the end serializes minutes of
model latency behind an already finished app.
...
- **Plain utility apps only** (converters, invoice/CRUD trackers, internal
  ...
  The injector builds that URL — do not paste it into `__root.tsx`. Optional
  theme colour: set `"color": "FF4D2E"` in `site.json` (6-digit hex, `#`
  optional). The injector appends `&color=` on the placeholder URL; a themed
  URL in `__root.tsx` is stripped. Custom cards ignore `color`.
  When the user renames the app, update `APP_NAME` (tab title) **and**
  `site.json` `title` (share card).

Live preview emits the same tags (preview `X-Forwarded-Host` is a valid
image host). On publish the request `Host` is enough — do **not** write a
`.env` for it. Card pixels and `site.json` update on the **next deploy**
...
No `/api/og` route, no runtime image renderer, no per-route cards. Do not
author or mutate `og:*` in `__root.tsx` — the platform injector owns those
tags. The card is one static site-wide image (`public/og.jpg` or the
placeholder service).
