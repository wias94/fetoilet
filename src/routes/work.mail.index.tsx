...
function WorkMailPage() {
  ...
  if (!user) return <Navigate to="/login" search={{ redirect: "/work/mail" }} />;

  return (
    <WorkShell>
      <p className="text-sm text-muted">客户发来的函件。点开就能回。</p>
      ...
      <ul className="mt-5 space-y-2">
        ...
        {rows?.length === 0 && <p className="py-12 text-center text-sm text-muted">还没有人写信</p>}
        {rows?.map((t) => (
          <li key={t.id}>
            <Link
              to="/work/mail/$id"
            ...
            >
              <div className="grid size-12 place-items-center rounded-xl bg-sunken text-sm">男</div>
